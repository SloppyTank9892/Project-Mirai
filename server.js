...
// Database setup
const db = new sqlite3.Database('./database/accounts.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
  if (err) {
  ...
  try {
    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], async (err, existingUser) => {
      ...
      db.run(
        ...
        [name, email, hashedPassword, userType || 'student', collegeName, course],
        function (err) {
          if (err) {
  ...
  try {
    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], async (err, user) => {
      ...
      req.login(user, (err) => {
        ...
        res.json({ message: 'Signed in successfully', user: { id: user.id, name: user.name, email: user.email } });
      });
    });
  } catch (error) {
...
app.post('/api/courses', requireAuth, [
